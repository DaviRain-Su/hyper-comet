...
/// ProofShip is local-first: no hosted edge is baked in. `ZERON_EDGE_URL`
...
/// explicitly configured via `ZERON_WORKOS_CLIENT_ID` (empty string forces dev
/// mode); a `ZERON_EDGE_TOKEN` dev bearer also keeps dev mode (smoke tests,
...
fn engine_config_from_env() -> zeron_engine::EngineConfig {
    ...
    zeron_engine::EngineConfig {
        ...
        // `ZERON_WORKOS_CLIENT_ID` opts into real WorkOS sign-in.
