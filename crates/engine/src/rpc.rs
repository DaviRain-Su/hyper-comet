...
use zeron_proto::{
    ...
    DeployScanResponse, DeploySendRequest, DeploySendResponse, DeploymentsResponse, EngineInfo,
    HarnessId, ImportLocalWalletRequest, NetworksResponse, OkxPutKeyRequest, OkxSetEnabledRequest,
    PutNetworksRequest, PutWalletsRequest, RemoveNetworkRequest, RemoveWalletRequest, ToolCall,
    UpsertNetworkRequest, UpsertWalletRequest, WalletConnectStartRequest,
    WalletConnectStartResponse, WalletsResponse, WorkspaceScope,
};
    ...
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        ...
        match method {
            ...
            methods::STUDIO_WC_START => {
                ...
                let started = self
                    .wallet_connect
                    .start(project_id, p.label, self.wallet_store.clone())
                    .await
                    .map_err(RpcError::Failed)?;
                RpcReply::value(&WalletConnectStartResponse {
                    url: started.url,
                    label: started.label,
                })
            }
            ...
            methods::OKX_PUT_KEY => {
                let p: OkxPutKeyRequest = parse_params(params)?;
                self.okx_store
                    .put(&p.api_key)
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
            ...
            methods::OKX_SET_ENABLED => {
                let p: OkxSetEnabledRequest = parse_params(params)?;
                self.okx_store
