...
impl CliResult {
    ...
    pub fn to_text(&self) -> String {
        serde_json::to_string(self).unwrap_or_else(|e| format!("{{\"ok\":false,\"stderr\":\"serialize: {e}\"}}"))
    }
}
...
pub async fn run_cli(args: &[String], timeout: Duration) -> CliResult {
    ...
    CliResult {
        ...
        stderr: stderr.clone(),
        parsed,
        error: if ok { None } else { Some(classify_error(&stderr, code)) },
    }
}
...
mod tests {
    ...
    #[test]
    fn error_classes() {
        assert_eq!(classify_error("PF-SRC-INVALID: bad", Some(3)), "src-invalid");
