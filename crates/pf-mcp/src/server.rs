...
use crate::cli::{run_cli, CliResult};
...
impl PfGate {
    ...
    async fn pf_check(
        ...
        if p.source.trim().is_empty() || p.module.trim().is_empty() {
            return Ok(wrap(CliResult::usage("pf_check requires source and module")));
        }
        let mut argv = vec![
            "check".into(),
            p.source.clone(),
        ...
        Ok(wrap(run_cli(&argv, timeout_or(CHECK_TIMEOUT, p.timeout_seconds)).await))
    }

    #[tool(
        name = "pf_build",
        description = "Compile a gate-passed ProgramV1 source into a sealed artifact set (bytecode + ABI + digest + gate report). Target defaults to `evm`. This tool never broadcasts to a network and never touches keys — deployment is done in the ProofShip app."
    )]
    async fn pf_build(
        &self,
        ...
        if p.source.trim().is_empty() || p.module.trim().is_empty() {
            return Ok(wrap(CliResult::usage("pf_build requires source and module")));
        }
        let target = p.target.as_deref().filter(|s| !s.trim().is_empty()).unwrap_or("evm");
        let mut argv = vec![
            "build".into(),
            p.source.clone(),
            "--module".into(),
        ...
        Ok(wrap(run_cli(&argv, timeout_or(BUILD_TIMEOUT, p.timeout_seconds)).await))
    }
    ...
    async fn pf_artifacts(
        ...
        let argv: Vec<String> = if let Some(dir) = p.output_dir.as_deref().filter(|s| !s.trim().is_empty()) {
            // Path form is forced so a target-id collision cannot hijack inspect.
            vec!["inspect".into(), "--output-dir".into(), dir.into(), "--json".into()]
