...
| 要求 | 状态 | 证据/动作 |
|---|---|---|
| 产品包含 AI 元素 | ✅ | 7 个 code agent lanes(Claude Code/Codex/Grok/Hermes/Pi/Cursor/OpenCode)在 Sessions 中经 ACP 层驱动; ProofForge skill 内嵌引擎,检测到工具链时自动注入 + 挂载 MCP 门禁(`crates/engine/src/proofforge.rs`);MCP 门禁是本仓库自维护的 `proofship-pf-mcp`(`crates/pf-mcp`,官方 rmcp SDK);配置 OnchainOS API key 后每个 session 自动挂载 OKX 官方托管 MCP(DEX 报价/流动性/approve/swap calldata,`crates/engine/src/okx.rs`) |
