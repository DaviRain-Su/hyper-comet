...
const here = dirname(fileURLToPath(import.meta.url)); // proofship/bridge
const repo = resolve(here, "../.."); // monorepo root
// Staging project root for drafts + gate runs (vertical-agnostic).
const proj = process.env.PROOFSHIP_PROJECT_ROOT ?? join(repo, "proofship/inbox");
const cliResolutionHelp = `product CLI missing. Resolve it by one of:
...
function resolvePfCli() {
  if (process.env.PF_CLI) return isExecutable(process.env.PF_CLI) ? process.env.PF_CLI : null;
  const pathDirs = String(process.env.PATH ?? "").split(":").filter(Boolean);
  for (const dir of pathDirs) {
    const candidate = join(dir, "proof-forge-next");
    if (isExecutable(candidate)) return candidate;
  }
  if (process.env.PROOF_FORGE_ROOT) {
    const candidate = join(process.env.PROOF_FORGE_ROOT, ".lake/build/bin/proof-forge-next");
    if (isExecutable(candidate)) return candidate;
  }
  const vendored = join(repo, "proofship/toolchain/bin/proof-forge-next");
  return isExecutable(vendored) ? vendored : null;
}
const cli = resolvePfCli();
...
if (!process.env.ELAN_TOOLCHAIN) {
  const pins = [join(repo, "proofship/toolchain/lean-toolchain")];
  if (process.env.PROOF_FORGE_ROOT) pins.push(join(process.env.PROOF_FORGE_ROOT, "lean-toolchain"));
  for (const pin of pins) {
    try {
      process.env.ELAN_TOOLCHAIN = readFileSync(pin, "utf8").trim();
      break;
    } catch {
      // pin file absent — try the next candidate
    }
  }
}
// Point the CLI at the vendored locked tool-root (solc/sbpf/wat2wasm) unless
// the operator already provides one.
if (!process.env.PROOF_FORGE_TOOL_ROOT) {
  const pfPlatform = `${process.platform === "darwin" ? "darwin" : process.platform}-${process.arch === "x64" ? "x86_64" : process.arch}`;
  const toolRoot = join(repo, "proofship/toolchain/tool-root", pfPlatform);
  if (existsSync(toolRoot)) process.env.PROOF_FORGE_TOOL_ROOT = toolRoot;
}
...
function pickModuleName(_nl) {
  // Vertical-agnostic: the module name is taken from the .lean file the agent
  // produces, not prescribed in the prompt.
  return "Contract";
}
...
async function createAgentDraft({ nl, lane }) {
  ...
  const systemPromptPath = process.env.PROOFSHIP_SYSTEM_PROMPT;
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/health") {
    json(res, 200, { ok: cli !== null && existsSync(cli), agent: DEFAULT_LANE });
    return;
  }
...
if (!existsSync(cli)) {
  console.error(`studio-bridge: ${cliResolutionHelp}`);
  process.exit(70);
}
